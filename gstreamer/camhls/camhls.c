 ...
 * gcc `pkg-config --cflags --libs glib-2.0` camhls.c -o camhls `pkg-config --cflags --libs gstreamer-1.0` -lglib-2.0 -levent
 */
...
int main()
{
    GstElement          *v4l2src, *capsfilter, *queue;
    GstElement          *x264enc, *h264parse, *rtph264pay, *udpsink;
    GMainLoop           *loop;
    GstBus              *bus;
    GstMessage          *msg;
    guint               bus_watch_id;
    ...
    if(!gst_element_link_many (v4l2src, capsfilter, queue, x264enc,
    ...
    g_snprintf (name, 100, "video/x-raw, format=(string)I420, \
        width=(int)%d, height=(int)%d, framerate=30/1", 640, 360);
